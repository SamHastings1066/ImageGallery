...
// TODO: need to update the GalleryList back in the galleryMVC every time this imageGallery in this one is updated. Consider making the other MVC this one's delegate. or else other option.
// TODO: Fix memory leak issue - every time you rearrange the collection view items the memory increases.
    ...
    private func fetchImage(for cell: ImageCollectionViewCell, from url: URL) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            if let urlContents = try? Data(contentsOf: url), let image = UIImage(data: urlContents) {
                // Update the UI therefore move back to the main thread
